...
class TransformedDistribution(distribution_lib.Distribution):
  ...
  @property
  def simple(self):
    '''silence event overriding for log_prob() and prob()
    which is now weirdly redirected towards
...
'''
  ...
  def _finish_log_prob_for_one_fiber(self, y, x, ildj, event_ndims):
    ...
    if self._is_maybe_event_override and not self.simple:
      log_prob = math_ops.reduce_sum(log_prob, self._reduce_event_indices)
    log_prob += math_ops.cast(ildj, log_prob.dtype)
    if self._is_maybe_event_override and isinstance(event_ndims, int) and not self.simple:
      log_prob.set_shape(
