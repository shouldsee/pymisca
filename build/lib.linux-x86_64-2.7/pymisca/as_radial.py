...
class RadialTransform(bijector.Bijector):
...
#     self._D = tf.constant(D,dtype='int32')
    ...
    with self._name_scope("init", values=[D]):
      ...
      self._D = D
      self.alpha = self._alpha = self._D / tf.constant(2.)
...
#     return math_ops.exp(math_ops.log1p(x * self.power) / self.power)
  ...
  def _inverse(self, y):
    ...
    x = tf.reduce_sum(tf.square(y),axis = -1)
    return x
...
'''
    pi = math_ops.acos(-1.)
    cst = - self.alpha * tf.log( pi ) + math_ops.lgamma(self.alpha)
    return cst
...
'''
    def __init__(self, distribution=None,D=None):
#         udist = tf.contrib.distributions.Uniform(low=0.,high=3.) 
        
        bjt = RadialTransform(D = D,debug=1)
